...
*/
  ...
  abstract class Typer(context0: Context) extends TyperDiagnostics with Adaptation with TyperContextErrors {
    ...
    def applyImplicitArgs(fun: Tree): Tree = fun.tpe match {
      case MethodType(params, _) =>
        ...
        for(param <- params) {
          ...
          val res = inferImplicit(fun, paramTp, true, false, context)(EV.DefaultExplanation)
          argResultsBuff += res
     ...
     */
    def inferView(tree: Tree, from: Type, to: Type, reportAmbiguous: Boolean, saveErrors: Boolean = true)(implicit e: EV.Explanation): Tree = {
      ...
      if (isPastTyper) EmptyTree
        ...
        case _ =>
          def wrapImplicit(from: Type, byName: Boolean): Tree = {
            val result = inferImplicit(tree, functionType(List(from), to), reportAmbiguous, true, context, saveErrors, byName)
          ...
          else wrapImplicit(appliedType(ByNameParamClass.typeConstructor, List(from)), true)
      }
    }
    ...
    protected def adapt(tree: Tree, mode: Int, pt: Type, original: Tree = EmptyTree)(implicit expl: EV.Explanation = EV.DefaultExplanation): Tree = {
      ...
      def adaptToImplicitMethod(mt: MethodType): Tree = {
        ...
        if (context.hasErrors)
          return setError(duplicateTree(tree))
      ...
      def instantiateToMethodType(mt: MethodType): Tree = {
        ...
        if (!meth.isConstructor && !meth.isMacro && isFunctionType(pt)) { // (4.2)
          ...
          EV.eventBlock(EV.EtaMethodTpeAdapt(meth, tree, pt), EV.AdaptDone(_)) {
      ...
      if (isAdaptableWithView(qual)) {
        ...
        inferView(qual, qual.tpe, searchTemplate, reportAmbiguous, saveErrors)(EV.AdaptQualToMemberInferView(qual, searchTemplate)) } match {
          case EmptyTree  =>
            ...
            qual
          case coercion   =>
            // TODO: Explanation can be easily expanded as
            //EV <<< EV.FoundCoercionAdapToMemberTyper(coercion)
            //EV >>> EV.TyperDone()
      ...
      if (templ.parents.isEmpty) List()
        ...
        while ((supertpt.tpe.typeSymbol ne null) && supertpt.tpe.typeSymbol.initialize.isTrait) {
          ...
          if (!supertpt1.isErrorTyped) {
            ...
            val supertpt1tt = TypeTree(supertpt1.tpe.parents.head)
            EV << EV.NewSuperTpePossiblyClass(supertpt, supertpt1tt)
            supertpt = supertpt1tt setPos supertpt.pos.focus
          }
        }
...
//        body = checkNoEscaping.locals(context.scope, restpe, body)
    ...
    def typedRefinement(stats: List[Tree]) {
      ...
      unit.toCheck += { () =>
        ...
        val stats1 = newTyper(c).typedStats(stats, NoSymbol)(EV.TypeRefinementStatement)
    ...
    def typedArgs(args0: List[Tree], mode: Int, formals0: List[Type], adapted0: List[Type])(implicit e: (Tree) => EV.Explanation): List[Tree] = {
      ...
      def loop(args: List[Tree], formals: List[Type], adapted: List[Type]): List[Tree] = {
        if (args.isEmpty || adapted.isEmpty) Nil
          ...
          val tree = typedArg(args.head, mode, typedMode, adapted.head)(e(args.head))
    ...
    def doTypedApply(tree: Tree, fun0: Tree, args: List[Tree], mode: Int, pt: Type): Tree = EV.eventBlockInform[Tree](EV.DoTypedApplyTyper(tree, fun0, args, mode, pt), EV.DoTypedApplyDone(_, _)){
      ...
      fun.tpe match {
        ...
        case ErrorType =>
          if (!tree.isErrorTyped) setError(duplicateTree(tree)) else tree
        ...
        case otpe if inPatternMode(mode) && unapplyMember(otpe).exists =>
          ...
          if (fun1.tpe.isErroneous) {
            ...
            if (sameLength(formals1, args)) {
              val args1 = typedArgs(args, mode, formals0, formals1)(_ => EV.DefaultExplanation)
...
/* --- end unapply  --- */
        ...
        def extract(fun: Tree, outerArgss: List[List[Tree]]):
          (Tree, List[List[Tree]]) = fun match {
            ...
            case _ =>
              ...
              (setError(duplicateTree(fun)), outerArgss)
          }
        extract(ann, List())
      }
    ...
    protected def typed1(tree: Tree, mode: Int, pt: Type)(implicit expl0: EV.Explanation): Tree = {
...
//        case Ident(name) => ??
      ...
      def typedIf(cond: Tree, thenp: Tree, elsep: Tree) = {
        ...
        if (elsep.isEmpty) { // in the future, should be unnecessary
          ...
          //EV << EV.IfBranchTyper("then", thenp, pt, true)
          var thenp1 = typed(thenp, pt)(EV.DefaultExplanation)
          //EV << EV.IfBranchTyper("else", elsep, pt, false)
      ...
      def typedMatch(tree: Tree, selector: Tree, cases: List[CaseDef]): Tree = {
        if (selector == EmptyTree) {
          ...
          typed1(atPos(tree.pos) { Function(params, body) }, mode, pt)(EV.DefaultExplanation)
        } else {
          ...
          if (isPastTyper || !opt.virtPatmat) {
            ...
            typed1(translated, mode, WildcardType)(EV.DefaultExplanation) setType owntype // TODO: get rid of setType owntype -- it should all typecheck
          }
        }
      }
       ...
       *  @return     ...
       */
      def tryTypedArgs(args: List[Tree], mode: Int)(implicit e: EV.Explanation): Either[List[AbsTypeError], List[Tree]] = {
      ...
      def typedApply(fun: Tree, args: List[Tree]) = {
        ...
        if (stableApplication && isPatternMode) {
          ...
          def onError(reportError: => Tree): Tree = {
            //EV << EV.TypedApplyErrorTyper(fun) //TODO redunant because of context errors?
            fun match {
              case Select(qual, name)
              if !isPatternMode && nme.isOpAssignmentName(name.decode) =>
                ...
                if (treeInfo.isVariableOrGetter(qual1)) {
                  ...
                  EV << EV.TypedApplyToAssignment(fun, qual1, name, args)
            ...
            /*
              ...
              */
              ...
              if (fun2.symbol == Array_apply && !res.isErrorTyped) {
                ...
                typed1(macroExpand(res), mode, pt)(EV.DefaultExplanation)
              else
                res
            case SilentTypeError(err) =>
      ...
      def typedSelect(qual: Tree, name: Name): Tree = {
        ...
        if (!reallyExists(sym)) {
          ...
          if (settings.Xexperimental.value && (qual.tpe.widen.typeSymbol isNonBottomSubClass DynamicClass)) {
            ...
            return typed1(util.trace("dynatype: ")(dynInvoke), mode, pt)(EV.DefaultExplanation)
          }
          ...
          val (result, accessibleError) = silent(_.makeAccessible(tree1, sym, qual.tpe, qual)) match {
            case SilentTypeError(err) =>
              if (err.kind != ErrorKinds.Access) {
                context issue err
                return setError(treeCopy.Select(tree, qual, name))
              } else (tree1, Some(err))
          ...
          val selection = result match {
            ...
            case _ if accessibleError.isDefined =>
              ...
              if (!qual1.isErrorTyped && (qual1 ne qual))
                ...
                setError(treeCopy.Select(tree, qual, name))
              }
            case _ =>
              result
          }
      ...
      tree match {
        ...
        case Typed(expr, Function(List(), EmptyTree)) =>
          ...
          typedEta(checkDead(typed1(expr, mode, pt)(EV.DefaultExplanation)))
          }
        ...
        case Typed(expr, tpt) =>
          ...
          val tpt1 = typedType(tpt, mode)(EV.TypeExplicitTypeAnnotation(tpt))
        ...
        case Apply(Block(stats, expr), args) =>
          ...
          typed1(atPos(tree.pos)(Block(stats, Apply(expr, args))), mode, pt)(EV.DefaultExplanation)
          }
        ...
        case SelectFromTypeTree(qual, selector) =>
          ...
          val qual1 = typedType(qual, mode)(EV.DefaultExplanation)
          if (qual1.tpe.isVolatile)
            TypeSelectionFromVolatileTypeError(tree, qual)
          else
            EV.eventBlock(EV.SelectTyper(qual1, selector, tree, pt), EV.TyperDone(_)) {
        ...
        case dc@TypeTreeWithDeferredRefCheck() =>
          typer1Event(EV.DeferredRefCheckTypeTyper(dc)){
          dc // TODO: should we re-type the wrapped tree? then we need to change TypeTreeWithDeferredRefCheck's representation to include the wrapped tree explicitly (instead of in its closure)
          }
        case tpt @ TypeTree() =>
    ...
    def findManifest(tp: Type, full: Boolean) = atPhase(currentRun.typerPhase) {
      inferImplicit(
        ...
        true, false, context)(EV.DefaultExplanation) //TODO
    }
