    ...
    def applyImplicitArgs(fun: Tree): Tree = fun.tpe match {
      case MethodType(params, _) =>
        ...
        for(param <- params) {
          ...
          val res = if (paramFailed) SearchFailure else inferImplicit(fun, paramTp, context.reportErrors, false, context)(EV.DefaultExplanation)
          argResultsBuff += res
          ...
          if (isPastTyper || !opt.virtPatmat) {
            ...
            (MatchTranslator(this)).translateMatch(selector1, cases1, owntype) match {
              ...
              case translated =>
                ...
                typed(translated, mode, WildcardType)(EV.DefaultExplanation) setType owntype
            }
          }
        }
      }
